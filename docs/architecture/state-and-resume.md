...
| `runner.ts` | Stage complete | `status`, `durationMs`, `error`, `summary` (emits `stage_complete` event on both success and error paths; `summary` is the agent's last `task_progress` description) |
