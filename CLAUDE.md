...
- **TUI**: Ink/React dashboard watches SQLite + stream logs (`src/tui/`). Renders at 10 FPS with 15-minute remount cycle to cap yoga-layout WASM memory. Adaptive poll interval: 500ms when active, 5s when gate-idle. Gate stages use static `⏸` icon (no spinner animation). sql.js WASM module reclaimed every ~15 min via `reclaimWasmMemory()`. Keyboard-scrollable detail log (↑↓ PgUp/PgDn). Active agents panel shows only in_progress agents with elapsed timers; 1-3 agents use horizontal columns, 4+ use compact rows. Sub-pipeline stages render inline with `├─` indent. Stage/phase start events include model, effort, inputs, and output metadata. Disable with `--no-tui` or `--headless`.
